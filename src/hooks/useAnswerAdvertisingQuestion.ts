...
export default function useAnswerAdvertisingQuestion({
  ...
  const handleAnswerAdvertisingQuestion = useCallback(async ({ comment, commentId }: { commentId: string; comment: string }) => {
    if (!breeder) return

    try {
      appDispatch(setIsLoading(true))

      await BackofficeBffService.postAdvertisingQuestionAnswer(
        breeder.id,
        poultryId,
        advertisingId,
        commentId,
